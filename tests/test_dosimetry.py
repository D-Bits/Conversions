...
from tests.functions.dosimetry_functions import(
    rems_to_sieverts, 
    sieverts_to_rems,
    rads_to_grays,
    grays_to_rads
...
class DosimetryTests(TestCase):
    ...
    def test_rems_to_sieverts(self):
        ...
        self.assertEqual(rems_to_sieverts(1), 0.01)

    # Test that 1 Sv equals 100 rems
    def test_sieverts_to_rems(self):

        self.assertEqual(sieverts_to_rems(1), 100)

    def test_rads_to_grays(self):

        self.assertEqual(rads_to_grays(1), 0.01)

    def test_grays_to_rads(self):

        self.assertEqual(grays_to_rads(1), 100)